---
name: documents/cloud.google.com/spanner
uri: https://cloud.google.com/spanner
title: Spanner
description: Build intelligent apps with a single database that combines relational, graph, key value, and search. No maintenance windows mean uninterrupted apps.
data_source: cloud.google.com
---

![Spanner](https://www.gstatic.com/bricks/image/BrbZw0tmDzakPJUIpEkrixRYaKNoruwLrG2Jmkc5mUFYuasV-43jhjxHF_wyqlvW6-rAp1Fqgy7S.png)

# Spanner

## Always-on database with virtually unlimited scale

Build intelligent, mission-critical apps on a single database that unifies relational, graph, key-value, and search. Run anywhere with [Spanner Omni](https://cloud.google.com/products/spanner/omni).

[Try Spanner free](https://console.cloud.google.com/freetrial?redirectPath=/spanner)

[Contact sales](https://cloud.google.com/contact)

Get started with a [90 day free trial instance](https://console.cloud.google.com/spanner/instances/landing-page).
...
Trust your data to a secure and compliant platform with Spanner. Enjoy centralized administration and control with [Database Center](https://cloud.google.com/database-center/docs/overview), simplifying your cloud database management. Spanner offers enterprise-grade security and controls, including data encryption at rest and in transit, granular access management through [Identity and Access Management (IAM)](https://cloud.google.com/spanner/docs/iam), and compliance with industry standards. Protect your data further with robust [backup](https://cloud.google.com/spanner/docs/backup) and restore capabilities, including [point-in-time recovery](https://cloud.google.com/spanner/docs/pitr) for operational peace of mind.
...
([PostgreSQL](https://youtu.be/W7R6db_sa0M), Google SQL)
...
##### Streamline MySQL and Cassandra modernization
...
Estimate your monthly Spanner costs, tailored to your workload.

[Estimate your costs](https://cloud.google.com/products/calculator)
...
[Watch the video](https://www.youtube.com/watch?v=DY2AR8Wzg3Y)
...
Spanner Omni is a downloadable version of Spanner delivering the same core capabilities with the added flexibility for customers to deploy it anywhere. To learn more, visit <https://cloud.google.com/products/spanner/omni>

#### Does Spanner use SQL?

Spanner provides two ANSI-based SQL dialects over the same rich set of capabilities: [GoogleSQL](https://cloud.google.com/spanner/docs/reference/standard-sql/lexical) and PostgreSQL. GoogleSQL shares syntax with BigQuery for teams standardizing their data management workflows. The PostgreSQL interface provides familiarity for teams who already know PostgreSQL and portability of schemas and queries to other PostgreSQL environments. For more information about the Spanner PostgreSQL interface, see our [documentation](https://cloud.google.com/spanner/docs/postgresql-interface).

#### How do I migrate databases to Spanner?

Migrating existing workloads to Spanner ensures a foundation for future growth, without having to compromise on reliability or price-performance. Spanner‘s multi-model interface is used today by innovative organizations across industries with operational workloads coming from relational databases, like MySQL and SQL Server, key-value stores, such as Cassandra or DynamoDB, along with graph, search, and document tools. The specific approach for a migration will depend on requirements around data volume, performance SLOs, and availability. [Spanner migration tool](https://github.com/GoogleCloudPlatform/spanner-migration-tool) provides end-to-end assessment, schema and data migration, and low-downtime cutover for sharded [MySQL](https://cloud.google.com/spanner/docs/migrating-mysql-to-spanner) and [Cassandra](https://cloud.google.com/spanner/docs/non-relational/spanner-for-cassandra-users) databases as well as assessment and migration for PostgreSQL. A qualified network of [technology and services partners](https://cloud.google.com/blog/products/databases/unlocking-the-power-of-spanner) can accelerate migrations from just about any source.
...
[Spanner Ecosystem](https://www.cloudspannerecosystem.dev/)

[Ask the community](https://stackoverflow.com/questions/tagged/google-cloud-spanner)
