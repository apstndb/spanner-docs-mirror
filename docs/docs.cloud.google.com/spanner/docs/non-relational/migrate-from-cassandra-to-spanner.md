...
For more information about how Spanner meets NoSQL database criteria, see [Spanner for non-relational workloads](https://docs.cloud.google.com/spanner/docs/non-relational/overview) .

## Migration constraints

For a successful migration from Cassandra to the Cassandra endpoint on Spanner, review [Spanner for Cassandra users](https://docs.cloud.google.com/spanner/docs/non-relational/cassandra-overview) to learn how Spanner architecture, data model, and data types differ from Cassandra. Carefully consider the functional differences between Spanner and Cassandra before you begin your migration.

## Migration process

The migration process is broken down into the following steps:

1.  [Convert your schema and data model](https://docs.cloud.google.com/spanner/docs/non-relational/migrate-from-cassandra-to-spanner#convert-schema-data-model) .
2.  [Set up dual writes for incoming data](https://docs.cloud.google.com/spanner/docs/non-relational/migrate-from-cassandra-to-spanner#set-up-dual-writes) .
3.  [Bulk export your historical data from Cassandra to Spanner](https://docs.cloud.google.com/spanner/docs/non-relational/migrate-from-cassandra-to-spanner#bulk-export-data-from-cassandra) .
4.  [Validate data to ensure data integrity throughout the migration process](https://docs.cloud.google.com/spanner/docs/non-relational/migrate-from-cassandra-to-spanner#data-validation) .
5.  [Point your application to Spanner instead of Cassandra](https://docs.cloud.google.com/spanner/docs/non-relational/migrate-from-cassandra-to-spanner#application-cutover) .
6.  Optional. [Perform reverse replication from Spanner to Cassandra](https://docs.cloud.google.com/spanner/docs/non-relational/migrate-from-cassandra-to-spanner#reverse-replication) .
...
1.  Review [Cassandra overview](https://docs.cloud.google.com/spanner/docs/non-relational/cassandra-overview) to understand the similarities and differences between Cassandra and Spanner data schemas and to learn how to map different data types.
...
![Apps send writes through a proxy to both Spanner and Cassandra](https://docs.cloud.google.com/static/spanner/docs/images/spanner-to-cassandra.png)

The live migration architecture has the following key components:

1.  **Origin:** Your source Cassandra database.

2.  **Target:** The destination Spanner database you're migrating to. It's assumed that you have already provisioned your [Spanner instance](https://docs.cloud.google.com/spanner/docs/create-manage-instances#create-instance) and [database](https://docs.cloud.google.com/spanner/docs/create-manage-databases#create-database) with a schema that's compatible with your Cassandra schema (with necessary adaptations for Spanner's data model and features).
...
Depending on the data consistency requirement, you can copy rows from Cassandra to Spanner to reconcile discrepancies identified during the validation phase. One way to do reconciliation is extending the tool used for full dataset validation, and copying the correct row from Cassandra to the target Spanner database if a mismatch is found. For more information, see [Implementation considerations](https://docs.cloud.google.com/spanner/docs/non-relational/migrate-from-cassandra-to-spanner#implementation-considerations) .
...
1.  Create a configuration change for your client application that lets it connect directly to your Spanner instance using one of the following methods:
      ...
      - Connect Cassandra to the [Cassandra Adapter](https://docs.cloud.google.com/spanner/docs/non-relational/connect-cassandra-adapter) running as a sidecar.
...
3.  Set up monitoring for your application to monitor for errors or performance issues. Monitor Spanner metrics using Cloud Monitoring. For more information, see [Monitor instances with Cloud Monitoring](https://docs.cloud.google.com/spanner/docs/monitoring-cloud) .
...
3.  [Read data within each partition](https://docs.cloud.google.com/spanner/docs/non-relational/migrate-from-cassandra-to-spanner#read-data-in-each-partition)
4.  [Fetch corresponding rows from Spanner](https://docs.cloud.google.com/spanner/docs/non-relational/migrate-from-cassandra-to-spanner#fetch-rows-from-spanner)
5.  [Design validation tools for extensibility](https://docs.cloud.google.com/spanner/docs/non-relational/migrate-from-cassandra-to-spanner#design-for-extensibility)
6.  [Report and log mismatches](https://docs.cloud.google.com/spanner/docs/non-relational/migrate-from-cassandra-to-spanner#report-log-mismatches)
...
This section describes how to enable and disable time to live (TTL) on Cassandra data in Spanner tables. For an overview, see [Time to live (TTL)](https://docs.cloud.google.com/spanner/docs/non-relational/cassandra-overview#time-to-live) .
...
## What's Next
  ...
  - See a comparison between Spanner and Cassandra in the [Cassandra overview](https://docs.cloud.google.com/spanner/docs/non-relational/cassandra-overview) .
