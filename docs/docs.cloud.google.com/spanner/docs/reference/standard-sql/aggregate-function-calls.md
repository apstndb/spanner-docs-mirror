...
Each aggregate function supports all or a subset of the aggregate function call syntax. You can use the following syntax to build an aggregate function:
  ...
  - `HAVING MAX` or `HAVING MIN` : Restricts the set of rows that the function aggregates by a maximum or minimum value. For details, see [HAVING MAX and HAVING MIN clause](https://docs.cloud.google.com/spanner/docs/reference/standard-sql/aggregate-function-calls#max_min_clause) .
  ...
  - `ORDER BY` : Specifies the order of the values.
      ...
      - If you're using floating point data types, see [Floating point semantics](https://docs.cloud.google.com/spanner/docs/reference/standard-sql/data-types#floating_point_semantics) on ordering and grouping.
...
This clause supports all [orderable data types](https://docs.cloud.google.com/spanner/docs/reference/standard-sql/data-types#data_type_properties) , except for `ARRAY` .
