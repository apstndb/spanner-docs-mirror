...
<tr class="odd">
<td><code dir="ltr" translate="no">spanner.commit_timestamp</code></td>
<td>An alias that maps to the data type <code dir="ltr" translate="no">timestamptz</code> . You can use this data type in combination with the <a href="https://docs.cloud.google.com/spanner/docs/reference/postgresql/functions#spanner-date-time-functions"><code dir="ltr" translate="no">spanner.pending_commit_timestamp()</code></a> function to write <a href="https://docs.cloud.google.com/spanner/docs/commit-timestamp">commit timestamps</a> .</td>
</tr>
<tr class="even">
...
## Commit timestamp type

Spanner maps the `spanner.commit_timestamp` type to the data type `timestamptz` with commit timestamp functionality enabled. The following example shows how to use `spanner.commit_timestamp` in a DDL statement for `CREATE TABLE` :
