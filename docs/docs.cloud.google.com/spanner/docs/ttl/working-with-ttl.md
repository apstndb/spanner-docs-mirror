...
Before adding TTL to your table, we recommend enabling [Spanner backup and restore](https://docs.cloud.google.com/spanner/docs/backup) . This lets you fully restore a database in case you accidentally delete your data with the TTL policy.

If you've enabled [point-in-time recovery](https://docs.cloud.google.com/spanner/docs/pitr) , you can view and restore deleted data—without a full restore from backup—if it's within the configured [version retention period](https://docs.cloud.google.com/spanner/docs/reference/rest/v1/projects.instances.databases#Database.FIELDS.version_retention_period) . For information on reading data in the past, see [Perform a stale read](https://docs.cloud.google.com/spanner/docs/reads#perform-stale-read) .

### Clean up old data

If this is the first time you're using TTL and you expect the first run to delete many rows, consider first cleaning up old data manually using [partitioned DML](https://docs.cloud.google.com/spanner/docs/dml-partitioned) . This gives you more control over the resource usage, instead of leaving it to the TTL background process. TTL runs at a low priority, ideal for incremental clean-up. However, this likely lengthens the time it takes to delete the initial set of rows in a busy database because Spanner's internal work scheduler prioritizes other work, such as user queries.
...
To change the database's schema, you must have the **spanner.databases.updateDdl** permission. For details, see [Access control for Spanner](https://docs.cloud.google.com/spanner/docs/iam#databases) .
...
Spanner has a [transaction size limit](https://docs.cloud.google.com/spanner/quotas#limits_for_creating_reading_updating_and_deleting_data) . Cascading deletes on large parent-child hierarchies with indexed columns could exceed these limits and cause one or more TTL operations to fail. For failed operations, TTL retries with smaller batches, down to a single parent row. However, large child hierarchies for even a single parent row could still exceed the mutation limit.

Failed operations are reported in [TTL metrics](https://docs.cloud.google.com/spanner/docs/ttl/monitoring-and-metrics) .
...
Spanner doesn't let you delete a column that's referenced by a row deletion policy. You must first [delete the row deletion policy](https://docs.cloud.google.com/spanner/docs/ttl/working-with-ttl#delete) before deleting the column.
