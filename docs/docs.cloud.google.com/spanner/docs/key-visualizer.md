...
Understanding data access patterns is critical for following [schema design best practices](https://docs.cloud.google.com/spanner/docs/schema-design) and avoiding hotspots. Hotspots occur when too many requests are sent to the same server which saturates the resources of the server and potentially causes high latencies.
