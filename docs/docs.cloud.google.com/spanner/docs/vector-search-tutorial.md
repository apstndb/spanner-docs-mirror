...
This tutorial demonstrates how to build a generative AI application using Spanner and [Gemini Enterprise Agent Platform](https://docs.cloud.google.com/gemini-enterprise-agent-platform/overview) .

This application lets you perform semantic similarity search, enabling you to find products that match a natural language query. It achieves this by using embeddings, which are numerical representations of text that capture the meaning and context of words. You'll use an Agent Platform model to generate these embeddings, and then store and search the embeddings in Spanner. This approach is particularly useful for use cases like product search, where users might describe what they want in natural language instead of specific keywords.

The following topics help you learn how to:

1.  [Create an Google Cloud project](https://docs.cloud.google.com/spanner/docs/vector-search-tutorial#before-you-begin)
2.  [Create a Spanner instance](https://docs.cloud.google.com/spanner/docs/vector-search-tutorial#create-instance)
3.  [Create a database](https://docs.cloud.google.com/spanner/docs/vector-search-tutorial#create-database)
4.  [Create an embedding model](https://docs.cloud.google.com/spanner/docs/vector-search-tutorial#create-embedding-model)
5.  [Load data into Spanner](https://docs.cloud.google.com/spanner/docs/vector-search-tutorial#load-data)
6.  [Generate embeddings for data](https://docs.cloud.google.com/spanner/docs/vector-search-tutorial#generate-embeddings)
7.  [Perform KNN vector similarity search](https://docs.cloud.google.com/spanner/docs/vector-search-tutorial#perform-vector-search)
8.  [Scale vector search with a vector index](https://docs.cloud.google.com/spanner/docs/vector-search-tutorial#scale-vector-search)
9.  [Clean up resources](https://docs.cloud.google.com/spanner/docs/vector-search-tutorial#clean-up)
...
The following example demonstrates how to register an Agent Platform [text embedding model](https://docs.cloud.google.com/gemini-enterprise-agent-platform/models/embeddings/get-text-embeddings) , which is then used to perform similarity search to find similar products in a database.
...
The previous vector search example uses exact, [K-nearest neighbors (KNN) vector search](https://docs.cloud.google.com/spanner/docs/find-k-nearest-neighbors) . The KNN vector distance functions (cosine distance, euclidean distance, and dot product) are helpful when you can query a specific subset of your Spanner data. Because KNN search calculates the exact distance between a query vector and all vectors in the database, it is efficient when you can partition the data. If your query needs to compare the query vector to all vectors in your database without any specific filters, and you can't divide the query into independent subqueries, you might experience performance bottlenecks if you use KNN. Approximate nearest neighbors (ANN) vector search becomes useful in these situations. For more information, see [Find approximate nearest neighbors](https://docs.cloud.google.com/spanner/docs/find-approximate-nearest-neighbors) .
