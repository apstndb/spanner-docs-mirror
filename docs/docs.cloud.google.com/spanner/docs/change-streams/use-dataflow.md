...
[Dataflow](https://docs.cloud.google.com/dataflow) is a serverless, fast, and cost-effective service that supports both stream and batch processing. It provides portability with processing jobs written using the open source [Apache Beam](https://beam.apache.org/) libraries and automates infrastructure provisioning and cluster management. Dataflow provides near real-time streaming when reading from change streams.
...
Dataflow templates are prebuilt Dataflow pipelines that implement common use cases. See [Dataflow templates](https://docs.cloud.google.com/spanner/docs/change-streams/use-dataflow#dataflow-templates) for an overview.

### Dataflow pipeline

A Spanner change streams Dataflow pipeline is composed of four main parts:

1.  [A Spanner database with a change stream](https://docs.cloud.google.com/spanner/docs/change-streams/use-dataflow#cloud-spanner-change-stream)
2.  [The SpannerIO connector](https://docs.cloud.google.com/spanner/docs/change-streams/use-dataflow#apache-beam-spannerio-connector)
3.  [User-defined transforms and sinks](https://docs.cloud.google.com/spanner/docs/change-streams/use-dataflow#user-defined-transforms)
4.  An [Apache Beam sink I/O writer](https://docs.cloud.google.com/spanner/docs/change-streams/use-dataflow#apache-sink-writer)

![image](https://docs.cloud.google.com/static/spanner/docs/change-streams/images/pipeline.svg)

#### Spanner change stream

For details on how to create a change stream, see [Create a change stream](https://docs.cloud.google.com/spanner/docs/change-streams/manage#create) .
...
[Dataflow templates](https://docs.cloud.google.com/dataflow/docs/concepts/dataflow-templates) provide a method to create Dataflow jobs based on prebuilt Docker images for common use-cases using the Google Cloud console, the Google Cloud CLI, or Rest API calls.
...
When you try to run a job from a Dataflow flex template without all the required permissions, then your job might fail with a [failed to read the result file error](https://docs.cloud.google.com/dataflow/docs/guides/troubleshoot-templates#read-job-file) or a [permission denied on resource error](https://docs.cloud.google.com/dataflow/docs/guides/troubleshoot-templates#permission-denied-on-resource) . For more information, see [Troubleshoot Flex Templates](https://docs.cloud.google.com/dataflow/docs/guides/troubleshoot-templates) .
...
To follow these steps, you need a Java development environment for Dataflow. For more information, see [Create a Dataflow pipeline using Java](https://docs.cloud.google.com/dataflow/docs/quickstarts/create-pipeline-java) .

### Create a change stream

For details about how to create a change stream, see [Create a change stream](https://docs.cloud.google.com/spanner/docs/change-streams/manage#create) . To continue with the next steps, you must have a Spanner database with a change stream configured.
...
This should only be used when [updating an existing pipeline](https://docs.cloud.google.com/spanner/docs/change-streams/use-dataflow#updating) .
...
This pipeline can be changed to order by transaction ID and commit timestamp. To do so, buffer records for each transaction ID / commit timestamp pair, instead of for each Spanner key. This requires modification of the code in [KeyByIdFn](https://docs.cloud.google.com/spanner/docs/change-streams/use-dataflow#keybyidfn) .
...
We also provide a Dataflow template for this use case: see [Connect change streams to Cloud Storage](https://docs.cloud.google.com/dataflow/docs/guides/templates/provided-streaming#cloud-spanner-change-streams-to-cloud-storage) .
...
We also provide a Dataflow template for this use case; see [Connect change streams to BigQuery](https://docs.cloud.google.com/dataflow/docs/guides/templates/provided-streaming#cloud-spanner-change-streams-to-bigquery) .
...
Dataflow provides several metrics to make sure your job is healthy, such as data freshness, system lag, job throughput, worker CPU utilization and more. You can find more information in [Using Monitoring for Dataflow pipelines](https://docs.cloud.google.com/dataflow/docs/guides/using-cloud-monitoring) .

For change streams pipelines, there are two main metrics that should be taken into account: the [system latency](https://docs.cloud.google.com/dataflow/docs/guides/using-monitoring-intf#system_latency_streaming) and the [data freshness](https://docs.cloud.google.com/dataflow/docs/guides/using-monitoring-intf#data_freshness_streaming) .
...
For more information, see [Consider a separate metadata database](https://docs.cloud.google.com/spanner/docs/change-streams/manage#why-metadata) .
...
The Spanner change streams connector does not support [Dataflow Snapshots](https://docs.cloud.google.com/dataflow/docs/guides/using-snapshots) .

#### Draining

The Spanner change streams connector does not support [draining a job](https://docs.cloud.google.com/dataflow/docs/guides/stopping-a-pipeline#drain) . It is only possible to cancel an existing job.

You can also [update an existing pipeline](https://docs.cloud.google.com/spanner/docs/change-streams/use-dataflow#updating) without needing to stop it.
