...
Applying the `SpannerIO.read` transform returns a consistent view of data by performing a strong read. Unless you specify otherwise, the result of the read is snapshotted at the time that you started the read. See [reads](https://docs.cloud.google.com/spanner/docs/reads#read_types) for more information about the different types of reads Spanner can perform.
...
You can also read a table using a specified secondary index. As with the [`readUsingIndex` API call](https://docs.cloud.google.com/spanner/docs/secondary-indexes#read-with-index) , the index must contain all of the data that appears in the query results.

To do so, specify the table as shown in the previous example, and specify the [index](https://docs.cloud.google.com/spanner/docs/secondary-indexes) that contains the needed column values using the [`SpannerIO.Read.withIndex`](https://beam.apache.org/documentation/sdks/javadoc/current/org/apache/beam/sdk/io/gcp/spanner/SpannerIO.Read.html#withIndex-java.lang.String-) method. The index must store all the columns that the transform needs to read. The base table's primary key is implicitly stored. For example, to read the `Songs` table using the index [`SongsBySongName`](https://docs.cloud.google.com/spanner/docs/secondary-indexes#add-index) , you use the following code:
...
A transform is guaranteed to be executed on a consistent snapshot of data. To control the [staleness](https://docs.cloud.google.com/spanner/docs/timestamp-bounds#timestamp_bound_types) of data, use the [`SpannerIO.Read.withTimestampBound`](https://beam.apache.org/documentation/sdks/javadoc/current/org/apache/beam/sdk/io/gcp/spanner/SpannerIO.Read.html#withTimestampBound-com.google.cloud.spanner.TimestampBound-) method. See [transactions](https://docs.cloud.google.com/spanner/docs/transactions) for more information.
