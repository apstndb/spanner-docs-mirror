...
The following operators are unary operators:
  ...
  - [Filter](#filter)
  - [Limit](#limit)
  - [Local split union](#local-split-union)
...
An *apply mutations* operator applies the mutations from a [Data Manipulation Language (DML) statement](/spanner/docs/dml-tasks) to the table. It's the top operator in a query plan for a DML statement.
...
A *create batch* operator batches its input rows into a sequence. A create batch operation usually occurs as a part of a [distributed cross apply](/spanner/docs/query-operators-distributed#distributed-cross-apply) operation. The input rows can be re-ordered during the batching. The number of input rows that get batched in each execution of the batch operator varies.

See the [distributed cross apply](/spanner/docs/query-operators-distributed#distributed-cross-apply) operator for an example of a create batch operator in an execution plan.
...
A *compute* operator produces output by reading its input rows and adding one or more additional columns that are computed using scalar expressions. See the [union all](/spanner/docs/query-operators-n-ary#union_all) operator for an example of a compute operator in an execution plan.
...
Spanner implements the predicate for singers whose last name starts with `  Rich  ` as a filter. The filter receives input from an index [scan](/spanner/docs/query-operators-leaf#scan) and outputs rows where `  LastName  ` starts with `  Rich  ` .
...
<tr class="odd">
<td>Condition</td>
<td>A predicate applied to each input row. When true the row is passed to the next operator; when false the row is discarded.</td>
</tr>
<tr class="even">
<td>Execution method</td>
<td>In Row execution, the operator processes one row at a time. In Batch execution, the operator processes a batch of rows at once.</td>
...
In this execution plan, the `  Random Id Assign  ` operator receives its input from a [distributed union](/spanner/docs/query-operators-distributed#distributed-union) operator, which receives its input from an [index scan](/spanner/docs/query-operators-leaf#scan) . The operator returns the rows with random ids and the `  Filter  ` operator then applies a scalar predicate on the random ids and returns approximately 10 percent of the rows.
...
In this execution plan, the sort operator receives its input rows from a [distributed union](/spanner/docs/query-operators-distributed#distributed-union) operator, sorts the input rows, and returns the sorted rows to a [serialize result](#serialize_result) operator.
...
A *union input* operator returns results to a [union all](/spanner/docs/query-operators-n-ary#union_all) operator. See the [union all](/spanner/docs/query-operators-n-ary#union_all) operator for an example of a union input operator in an execution plan.
