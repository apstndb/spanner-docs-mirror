...
You can use any model with the emulator to generate predictions. You can also use a model from [Model Garden](https://docs.cloud.google.com/gemini-enterprise-agent-platform/models/model-garden/explore-models) or a model deployed to your [Agent Platform](https://docs.cloud.google.com/gemini-enterprise-agent-platform/machine-learning/general/deployment) endpoint. Since the emulator doesn't connect to Agent Platform, the emulator can't verify the model or its schema for any model used from Model Garden or deployed to the Agent Platform endpoints.
...
You can either [install the emulator locally](https://docs.cloud.google.com/spanner/docs/emulator#install) or set it up using the [GitHub repository](https://github.com/GoogleCloudPlatform/cloud-spanner-emulator) .

### Select a model

When you use the `ML.PREDICT` (for GoogleSQL) or the `ML_PREDICT_ROW` (for PostgreSQL) function, you must specify the location of the ML model. You can use any trained model. If you select a model that is running in [Model Garden](https://docs.cloud.google.com/gemini-enterprise-agent-platform/models/model-garden/explore-models) or a model that is deployed to your [Agent Platform](https://docs.cloud.google.com/gemini-enterprise-agent-platform/machine-learning/general/deployment) endpoint, you must provide the `input` and `output` values for these models.
...
You can use the emulator to generate predictions using the [Spanner ML predict functions](https://docs.cloud.google.com/spanner/docs/ml#ml-functions) .
...
You can use a custom callback function to implement selected model behaviors, and to transform specific model inputs to outputs. The following example uses the `gemini-pro` model from Model Garden and the Spanner emulator to generate predictions using a custom callback.
