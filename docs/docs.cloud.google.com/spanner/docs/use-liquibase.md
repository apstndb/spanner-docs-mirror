...
To use Liquibase with GoogleSQL-dialect databases, you have to install the Spanner Liquibase extension. For PostgreSQL-dialect databases, Liquibase can use its built-in PostgreSQL support in conjunction with [PGAdapter](https://docs.cloud.google.com/spanner/docs/pgadapter) .
...
1.  Ensure that PGAdapter is started and running on the machine where you install Liquibase. For more information, see [Start PGAdapter](https://docs.cloud.google.com/spanner/docs/pgadapter-start) .
