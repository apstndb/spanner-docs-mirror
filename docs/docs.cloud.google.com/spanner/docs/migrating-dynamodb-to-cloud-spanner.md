...
![Architectural diagram of migration components](https://docs.cloud.google.com/static/spanner/docs/images/migrating-dynamodb-to-cloud-spanner-1architecture.svg)
...
When you finish the tasks that are described in this document, you can avoid continued billing by deleting the resources that you created. For more information, see [Clean up](https://docs.cloud.google.com/spanner/docs/migrating-dynamodb-to-cloud-spanner#clean-up) .
...
An Amazon DynamoDB primary key establishes uniqueness and can be either a hash key or a combination of a hash key plus a range key. This tutorial starts by demonstrating the migration of a Amazon DynamoDB table whose primary key is a hash key. This hash key becomes the primary key of your Spanner table. Later, in the [section on interleaved tables](https://docs.cloud.google.com/spanner/docs/migrating-dynamodb-to-cloud-spanner#interleaved_indexes) , you model a situation where an Amazon DynamoDB table uses a primary key composed of a hash key and a range key.

### Secondary indexes

Both Amazon DynamoDB and Spanner support creating an index on a non-primary key attribute. Take note of any secondary indexes in your Amazon DynamoDB table so that you can create them on your Spanner table, which is covered in a [later section of this tutorial](https://docs.cloud.google.com/spanner/docs/migrating-dynamodb-to-cloud-spanner#apply_secondary_indexes) .
...
When the batch import job is complete, you set up a streaming job to write ongoing updates from the source table into Spanner. You [subscribe](https://docs.cloud.google.com/pubsub/docs/subscriber) to the events from Pub/Sub and write them to Spanner
...
![User table compared to orders table](https://docs.cloud.google.com/static/spanner/docs/images/migrating-dynamodb-to-cloud-spanner-6table-mapping.svg)
...
You can set up interleaved indexes in Spanner. The secondary indexes discussed in the [previous section](https://docs.cloud.google.com/spanner/docs/migrating-dynamodb-to-cloud-spanner#apply_secondary_indexes) are at the root of the database hierarchy, and they use indexes the same way as a conventional database. An interleaved index is within the context of its interleaved row. See [index options](https://docs.cloud.google.com/spanner/docs/whitepapers/optimizing-schema-design#index_options) for more details about where to apply interleaved indexes.
