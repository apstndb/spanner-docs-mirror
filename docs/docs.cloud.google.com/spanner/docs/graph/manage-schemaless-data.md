...
For more information about when to use schemaless data management, see [Considerations for schemaless data management](https://docs.cloud.google.com/spanner/docs/graph/manage-schemaless-data#when-not-to-use-schemaless-data-management) .
...
For more information about limitations when using dynamic labels, see [Limitations](https://docs.cloud.google.com/spanner/docs/graph/manage-schemaless-data#limitations) .
...
You can query schemaless graph data using [Graph Query Language (GQL)](https://docs.cloud.google.com/spanner/docs/reference/standard-sql/graph-intro) . You can use the sample queries in the [Spanner Graph Query overview](https://docs.cloud.google.com/spanner/docs/graph/queries-overview) and [GQL reference](https://docs.cloud.google.com/spanner/docs/reference/standard-sql/graph-query-statements) with limited modifications.
...
The following example shows how to access nested properties with the JSON field [access operator](https://docs.cloud.google.com/spanner/docs/reference/standard-sql/operators#field_access_operator) .
...
Spanner Graph maps data from tables to graph nodes and edges. When you change input table data, this change directly causes mutations to the corresponding graph data. For more information about graph data mutation, see [Insert, update, or delete Spanner Graph data](https://docs.cloud.google.com/spanner/docs/graph/insert-update-delete-data) .
...
You must store label string values as lowercase for matching. We recommend that you enforce this rule either in the application code or using schema [constraints](https://docs.cloud.google.com/spanner/docs/graph/manage-schemaless-data#enforce-lowercase-label-values-and-property-names) .
...
You must store property names in lowercase. We recommend that you enforce this rule either in the application code or using schema [constraints](https://docs.cloud.google.com/spanner/docs/graph/manage-schemaless-data#enforce-lowercase-label-values-and-property-names) .
...
Spanner supports schema objects such as [check constraints](https://docs.cloud.google.com/spanner/docs/check-constraint/how-to) to enforce label and property data integrity. This section lists recommendations for check constraints that you can use with schemaless data.
...
Because label and property names must be [stored as lowercase values](https://docs.cloud.google.com/spanner/docs/graph/manage-schemaless-data#property-names-must-be-lowercase) , do either of the following:

  - Enforce the check in application logic.
  - Create [check constraints](https://docs.cloud.google.com/spanner/docs/check-constraint/how-to) in the schema.
...
Use the schema examples in this section as templates to create your own schemas. Key schema components include the following:
  ...
  - Optional: reverse edge traversal index to boost [reverse traversal performance](https://docs.cloud.google.com/spanner/docs/graph/best-practices-designing-schema#optimize-reverse-edge-traversal-secondary-index)
  - Optional: label index to boost performance of queries by labels
  - Optional: schema constraints to enforce [lowercase labels](https://docs.cloud.google.com/spanner/docs/graph/manage-schemaless-data#labels-must-be-lowercase) and [property names](https://docs.cloud.google.com/spanner/docs/graph/manage-schemaless-data#property-names-must-be-lowercase)
...
## What's next
  ...
  - To learn more about JSON, see [Modify JSON data](https://docs.cloud.google.com/spanner/docs/working-with-json#modify) and [JSON functions list](https://docs.cloud.google.com/spanner/docs/reference/standard-sql/json_functions#categories) .
