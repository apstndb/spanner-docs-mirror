...
![Spanner Proof of Concept](https://docs.cloud.google.com/static/spanner/docs/images/poc.svg)
...
![The lifecycle includes planning, configuration, sizing, data loading, load testing, monitoring, and optimization.](https://docs.cloud.google.com/static/spanner/docs/images/poc-lifecycle.svg)

## Plan your POC

![Planning step](https://docs.cloud.google.com/static/spanner/docs/images/poc-plan.svg)
...
| Recovery Point Objective (RPO) guarantee in case of a regional outage | 0                                                                                                                                                                      |
| Latency limit for most critical transactions                          | p50 below 20ms                                                                                                                                                         |
| Latency for our most critical user-facing queries                     | p50 below 100ms                                                                                                                                                        |
| Scalability                                                           | Demonstrate that scaling up from 10,000 transactions per second to 100,000 transactions per second with p50 latency below 20ms is possible over the course of one hour |
...
![Spanner configuration step](https://docs.cloud.google.com/static/spanner/docs/images/poc-config-spanner.svg)
...
![Sizing Spanner step](https://docs.cloud.google.com/static/spanner/docs/images/poc-sizing.svg)
...
Starting with a reasonable assumption lets you establish a baseline and incrementally scale based on observed performance. You can use the sizing guidance from Spanner's [reference benchmarks](https://docs.cloud.google.com/spanner/docs/performance) to establish a baseline instance configuration.
...
Typical workload patterns, such as consistent traffic versus fluctuating demand, should influence your sizing approach. When you enable [autoscaling](https://docs.cloud.google.com/spanner/docs/autoscaling-overview) , Spanner provisions its compute resource capacity dynamically to match workload intensity.

## Schema design

![Schema design step](https://docs.cloud.google.com/static/spanner/docs/images/poc-schema-design.svg)
...
![Create a draft schema in the Console.](https://docs.cloud.google.com/static/spanner/docs/images/poc-schema-design-example.svg)
...
If no sample data is available, you can use synthetic data generation tools such as [JMeter from Machmeter](https://docs.cloud.google.com/spanner/docs/proof-of-concept-playbook#jmeter) and [QuickPerf](https://docs.cloud.google.com/spanner/docs/proof-of-concept-playbook#quickperf) to help you create datasets tailored to your schema and use case. For more information, see [Generate sample data](https://docs.cloud.google.com/spanner/docs/proof-of-concept-playbook#generate-sample-data) .

### Bring your own data

![Bring your own data step](https://docs.cloud.google.com/static/spanner/docs/images/poc-bring-data.svg)
...
| **Any JDBC**   | [JDBC to Spanner](https://docs.cloud.google.com/dataflow/docs/guides/templates/provided/sourcedb-to-spanner)           | manual              | Data type conversion only         | large         |
| **CSV**        | [CSV to Spanner](https://docs.cloud.google.com/dataflow/docs/guides/templates/provided/cloud-storage-to-cloud-spanner) | manual              | Data type conversion only         | large         |
|                | [BigQuery reverse ETL](https://docs.cloud.google.com/bigquery/docs/export-to-spanner)                                  | manual              | Complex transformations supported | large         |
| **Avro**       | [Avro to Spanner](https://docs.cloud.google.com/dataflow/docs/guides/templates/provided/avro-to-cloud-spanner)         | manual              | Data type conversion only         | large         |
|                | [BigQuery reverse ETL](https://docs.cloud.google.com/bigquery/docs/export-to-spanner)                                  | manual              | Complex transformations supported | large         |
| **JSON**       | [BigQuery reverse ETL](https://docs.cloud.google.com/bigquery/docs/export-to-spanner)                                  | manual              | Complex transformations supported | large         |

#### BigQuery reverse ETL to Spanner

[BigQuery reverse ETL to Spanner](https://docs.cloud.google.com/bigquery/docs/export-to-spanner) lets you [quickly ingest a wide range of data sources](https://cloud.google.com/blog/topics/developers-practitioners/bigquery-explained-data-ingestion) and transform them into BigQuery tables using SQL. You can then [export data from the BigQuery table into a Spanner table](https://docs.cloud.google.com/bigquery/docs/export-to-spanner#export_data) . It's particularly useful for semi-structured data, such as JSON, which often originates as exports from NoSQL data sources. While BigQuery has automatic schema detection in place, Spanner schema creation is manual, requiring that you define the schema before loading data.
...
[Dataflow](https://docs.cloud.google.com/dataflow/docs/overview) is a fully managed service designed for scalable data processing, making it a suitable choice for loading large quantities of data.
...
![Generate sample data step.](https://docs.cloud.google.com/static/spanner/docs/images/poc-sample-data.svg)
...
![Load testing step](https://docs.cloud.google.com/static/spanner/docs/images/poc-load-testing.svg)
...
![Monitoring step](https://docs.cloud.google.com/static/spanner/docs/images/poc-monitoring.svg)
...
**[Key Visualizer](https://docs.cloud.google.com/spanner/docs/key-visualizer)** provides a visual representation of database usage patterns over time across the entire key space of both tables and indexes. It generates heatmaps that show read and write activity, highlighting areas of high intensity and potentially [problematic patterns](https://docs.cloud.google.com/spanner/docs/key-visualizer/patterns) . During a POC, this tool helps to validate schema design and identify potential scalability limitations. As the load increases, you can observe how the workload is distributed across your key space and respective tables and indexes.

**[Introspection tables](https://docs.cloud.google.com/spanner/docs/introspection)** , primarily its system of `  Spanner_SYS  ` tables, provide a wealth of information about the database's internal state and performance. These tables expose detailed statistics on query execution, transaction behavior, lock contention, and schema details. During a POC load test, these introspection tables provide a data-driven approach to performance diagnostics, beyond what the previously mentioned insights tools offers. For example, you can use them to troubleshoot the root cause of otherwise hard to detect [lock conflicts in your database](https://docs.cloud.google.com/spanner/docs/introspection/lock-statistics#apply_best_practices_to_reduce_lock_contention) and derive actionable insights for optimization.

## Optimization

![Optimization step](https://docs.cloud.google.com/static/spanner/docs/images/poc-optimization.svg)
