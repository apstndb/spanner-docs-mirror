---
name: documents/docs.cloud.google.com/sdk/gcloud/reference/beta/spanner/backup-schedules/create
uri: https://docs.cloud.google.com/sdk/gcloud/reference/beta/spanner/backup-schedules/create
title: gcloud beta spanner backup-schedules create
...
gcloud beta spanner backup-schedules create - create a Cloud Spanner backup schedule

SYNOPSIS

`gcloud beta spanner backup-schedules create` ( `  BACKUP_SCHEDULE  ` : `  --database  ` = `  DATABASE  ` `  --instance  ` = `  INSTANCE  ` ) `  --backup-type  ` = \[ `  BACKUP_TYPE  ` \] `  --cron  ` = `  CRON  ` `  --retention-duration  ` = `  RETENTION_DURATION  ` \[ `  --encryption-type  ` = `  ENCRYPTION_TYPE  ` `  --kms-keys  ` =\[ `  KMS_KEYS  ` , …\] | \[ `  --kms-key  ` = `  KMS_KEY  ` : `  --kms-keyring  ` = `  KMS_KEYRING  ` `  --kms-location  ` = `  KMS_LOCATION  ` `  --kms-project  ` = `  KMS_PROJECT  ` \]\] \[ `  GCLOUD_WIDE_FLAG …  ` \]
