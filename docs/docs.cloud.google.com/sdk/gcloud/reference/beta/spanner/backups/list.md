---
name: documents/docs.cloud.google.com/sdk/gcloud/reference/beta/spanner/backups/list
uri: https://docs.cloud.google.com/sdk/gcloud/reference/beta/spanner/backups/list
title: gcloud beta spanner backups list
...
`gcloud beta spanner backups list` \[ `  --database  ` = `  DATABASE  ` \] \[ `  --instance  ` = `  INSTANCE  ` \] \[ `  --filter  ` = `  EXPRESSION  ` \] \[ `  --limit  ` = `  LIMIT  ` \] \[ `  --page-size  ` = `  PAGE_SIZE  ` \] \[ `  --sort-by  ` =\[ `  FIELD  ` , …\]\] \[ `  GCLOUD_WIDE_FLAG …  ` \]
