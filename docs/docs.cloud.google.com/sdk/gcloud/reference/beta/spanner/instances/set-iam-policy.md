---
name: documents/docs.cloud.google.com/sdk/gcloud/reference/beta/spanner/instances/set-iam-policy
uri: https://docs.cloud.google.com/sdk/gcloud/reference/beta/spanner/instances/set-iam-policy
title: gcloud beta spanner instances set-iam-policy
...
`gcloud beta spanner instances set-iam-policy` \[ `  INSTANCE  ` \] `  POLICY_FILE  ` \[ `  GCLOUD_WIDE_FLAG …  ` \]

DESCRIPTION

`(BETA)` Set the IAM policy for a Cloud Spanner instance given a instance ID and a file encoded in JSON or YAML that contains the IAM policy.
...
The following command reads an IAM policy defined in a JSON file `policy.json` and sets it for a spanner instance with the ID `example-instance` :
    ...
    gcloud beta spanner instances set-iam-policy example-instance policy.json
