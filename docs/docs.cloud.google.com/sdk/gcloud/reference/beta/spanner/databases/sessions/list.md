---
name: documents/docs.cloud.google.com/sdk/gcloud/reference/beta/spanner/databases/sessions/list
uri: https://docs.cloud.google.com/sdk/gcloud/reference/beta/spanner/databases/sessions/list
title: gcloud beta spanner databases sessions list
...
`gcloud beta spanner databases sessions list` ( `  --database  ` = `  DATABASE  ` : `  --instance  ` = `  INSTANCE  ` ) \[ `  --server-filter  ` = `  SERVER_FILTER  ` \] \[ `  --filter  ` = `  EXPRESSION  ` \] \[ `  --limit  ` = `  LIMIT  ` \] \[ `  --page-size  ` = `  PAGE_SIZE  ` \] \[ `  --sort-by  ` =\[ `  FIELD  ` , …\]\] \[ `  --uri  ` \] \[ `  GCLOUD_WIDE_FLAG …  ` \]
...
To list the sessions for a given Cloud Spanner database, run:
    ...
    gcloud beta spanner databases sessions list --instance=my-instance-id --database=my-database-id

REQUIRED FLAGS

Database resource - The Cloud Spanner database in which to list sessions. The arguments in this group can be used to specify the attributes of this resource. (NOTE) Some attributes are not given arguments in this group but can be set in other ways.
