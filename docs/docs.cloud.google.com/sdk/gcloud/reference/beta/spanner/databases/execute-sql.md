---
name: documents/docs.cloud.google.com/sdk/gcloud/reference/beta/spanner/databases/execute-sql
uri: https://docs.cloud.google.com/sdk/gcloud/reference/beta/spanner/databases/execute-sql
title: gcloud beta spanner databases execute-sql
...
`gcloud beta spanner databases execute-sql` ( `  DATABASE  ` : `  --instance  ` = `  INSTANCE  ` ) `  --sql  ` = `  SQL  ` \[ `  --database-role  ` = `  DATABASE_ROLE  ` \] \[ `  --enable-partitioned-dml  ` \] \[ `  --priority  ` = `  PRIORITY  ` \] \[ `  --query-mode  ` = `  QUERY_MODE  ` ; default="NORMAL"\] \[ `  --timeout  ` = `  TIMEOUT  ` ; default="10m"\] \[ `  --read-timestamp  ` = `  TIMESTAMP  ` | `  --strong  ` \] \[ `  GCLOUD_WIDE_FLAG …  ` \]
...
To execute a SQL SELECT statement against example-database under example-instance, run:
    ...
    gcloud beta spanner databases execute-sql example-database --instance=example-instance --sql='SELECT * FROM MyTable WHERE MyKey = 1'
