---
name: documents/docs.cloud.google.com/sdk/gcloud/reference/beta/spanner/databases/ddl/update
uri: https://docs.cloud.google.com/sdk/gcloud/reference/beta/spanner/databases/ddl/update
title: gcloud beta spanner databases ddl update
...
`gcloud beta spanner databases ddl update` ( `  DATABASE  ` : `  --instance  ` = `  INSTANCE  ` ) \[ `  --async  ` \] \[ `  --ddl  ` = `  DDL  ` \] \[ `  --ddl-file  ` = `  DDL_FILE  ` \] \[ `  --proto-descriptors-file  ` = `  PROTO_DESCRIPTORS_FILE  ` \] \[ `  GCLOUD_WIDE_FLAG …  ` \]
...
To add a column to a table in the given Cloud Spanner database, run:
    ...
    gcloud beta spanner databases ddl update my-database-id --instance=my-instance-id --ddl='ALTER TABLE test_table ADD COLUMN a INT64'
