...
`  gcloud spanner operations cancel  ` `  OPERATION-ID  ` ( `  --instance  ` = `  INSTANCE  ` | `  --instance-config  ` = `  INSTANCE_CONFIG  ` ) \[ `  --backup  ` = `  BACKUP  ` \] \[ `  --database  ` = `  DATABASE  ` \] \[ `  --instance-partition  ` = `  INSTANCE_PARTITION  ` \] \[ `  GCLOUD_WIDE_FLAG …  ` \]
