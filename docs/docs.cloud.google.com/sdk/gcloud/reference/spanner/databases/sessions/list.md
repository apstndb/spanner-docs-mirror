---
name: documents/docs.cloud.google.com/sdk/gcloud/reference/spanner/databases/sessions/list
uri: https://docs.cloud.google.com/sdk/gcloud/reference/spanner/databases/sessions/list
title: gcloud spanner databases sessions list
...
`gcloud spanner databases sessions list` ( `  --database  ` = `  DATABASE  ` : `  --instance  ` = `  INSTANCE  ` ) \[ `  --server-filter  ` = `  SERVER_FILTER  ` \] \[ `  --filter  ` = `  EXPRESSION  ` \] \[ `  --limit  ` = `  LIMIT  ` \] \[ `  --page-size  ` = `  PAGE_SIZE  ` \] \[ `  --sort-by  ` =\[ `  FIELD  ` , …\]\] \[ `  --uri  ` \] \[ `  GCLOUD_WIDE_FLAG …  ` \]
