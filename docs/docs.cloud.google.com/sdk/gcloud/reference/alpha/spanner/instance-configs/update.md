---
name: documents/docs.cloud.google.com/sdk/gcloud/reference/alpha/spanner/instance-configs/update
uri: https://docs.cloud.google.com/sdk/gcloud/reference/alpha/spanner/instance-configs/update
title: gcloud alpha spanner instance-configs update
...
`gcloud alpha spanner instance-configs update` `  INSTANCE_CONFIG  ` \[ `  --async  ` \] \[ `  --display-name  ` = `  DISPLAY_NAME  ` \] \[ `  --etag  ` = `  ETAG  ` \] \[ `  --update-labels  ` =\[ `  KEY  ` = `  VALUE  ` , …\]\] \[ `  --validate-only  ` \] \[ `  --clear-labels  ` | `  --remove-labels  ` =\[ `  KEY  ` , …\]\] \[ `  GCLOUD_WIDE_FLAG …  ` \]
...
To update display name of a custom Cloud Spanner instance configuration 'custom-instance-config', run:
    ...
    gcloud alpha spanner instance-configs update custom-instance-config --display-name=nam3-RO-us-central1
...
To modify the instance config 'custom-instance-config' by adding label 'k0', with value 'value1' and label 'k1' with value 'value2' and removing labels with key 'k3', run:
    ...
    gcloud alpha spanner instance-configs update custom-instance-config --update-labels=k0=value1,k1=value2 --remove-labels=k3

To clear all labels of a custom Cloud Spanner instance configuration 'custom-instance-config', run:

    gcloud alpha spanner instance-configs update custom-instance-config --clear-labels

To remove an existing label of a custom Cloud Spanner instance configuration 'custom-instance-config', run:

    gcloud alpha spanner instance-configs update custom-instance-config --remove-labels=KEY1,KEY2
...
At most one of these can be specified:
  ...
  - `--clear-labels`  
    ...
    To remove all existing labels and create two new labels, `  foo  ` and `  baz  ` :
        ...
        gcloud alpha spanner instance-configs update --clear-labels --update-labels foo=bar,baz=qux
