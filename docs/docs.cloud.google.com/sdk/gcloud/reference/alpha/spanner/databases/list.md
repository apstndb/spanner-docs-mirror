...
`  gcloud alpha spanner databases list  ` \[ `  --instance  ` = `  INSTANCE  ` \] \[ `  --filter  ` = `  EXPRESSION  ` \] \[ `  --limit  ` = `  LIMIT  ` \] \[ `  --page-size  ` = `  PAGE_SIZE  ` \] \[ `  --sort-by  ` =\[ `  FIELD  ` , …\]\] \[ `  --uri  ` \] \[ `  GCLOUD_WIDE_FLAG …  ` \]
...
``` text
gcloud alpha spanner databases list --instance=my-instance-id
```
