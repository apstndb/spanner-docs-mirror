---
name: documents/docs.cloud.google.com/sdk/gcloud/reference/alpha/spanner/backup-schedules/create
uri: https://docs.cloud.google.com/sdk/gcloud/reference/alpha/spanner/backup-schedules/create
title: gcloud alpha spanner backup-schedules create
...
gcloud alpha spanner backup-schedules create - create a Cloud Spanner backup schedule

SYNOPSIS

`gcloud alpha spanner backup-schedules create` ( `  BACKUP_SCHEDULE  ` : `  --database  ` = `  DATABASE  ` `  --instance  ` = `  INSTANCE  ` ) `  --backup-type  ` = \[ `  BACKUP_TYPE  ` \] `  --cron  ` = `  CRON  ` `  --retention-duration  ` = `  RETENTION_DURATION  ` \[ `  --encryption-type  ` = `  ENCRYPTION_TYPE  ` `  --kms-keys  ` =\[ `  KMS_KEYS  ` , …\] | \[ `  --kms-key  ` = `  KMS_KEY  ` : `  --kms-keyring  ` = `  KMS_KEYRING  ` `  --kms-location  ` = `  KMS_LOCATION  ` `  --kms-project  ` = `  KMS_PROJECT  ` \]\] \[ `  GCLOUD_WIDE_FLAG …  ` \]
...
To create a Cloud Spanner backup schedule, run:
    ...
    gcloud alpha spanner backup-schedules create backup-schedule-id --instance=instance-id --database=database-id --cron="0 2 * * *" --retention-duration=2w --backup-type=full-backup

POSITIONAL ARGUMENTS

Backup schedule resource - The Cloud Spanner backup schedule to create. The arguments in this group can be used to specify the attributes of this resource. (NOTE) Some attributes are not given arguments in this group but can be set in other ways.
