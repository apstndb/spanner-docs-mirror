---
name: documents/docs.cloud.google.com/sdk/gcloud/reference/alpha/spanner/samples/backend
uri: https://docs.cloud.google.com/sdk/gcloud/reference/alpha/spanner/samples/backend
title: gcloud alpha spanner samples backend
...
gcloud alpha spanner samples backend - run the backend gRPC service for the given Cloud Spanner sample app

SYNOPSIS

`gcloud alpha spanner samples backend` `  APPNAME  ` `  --instance-id  ` = `  INSTANCE_ID  ` \[ `  --database-id  ` = `  DATABASE_ID  ` \] \[ `  --duration  ` = `  DURATION  ` ; default="1h"\] \[ `  --port  ` = `  PORT  ` \] \[ `  GCLOUD_WIDE_FLAG …  ` \]
