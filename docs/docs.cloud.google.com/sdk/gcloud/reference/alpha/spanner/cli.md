---
name: documents/docs.cloud.google.com/sdk/gcloud/reference/alpha/spanner/cli
uri: https://docs.cloud.google.com/sdk/gcloud/reference/alpha/spanner/cli
title: gcloud alpha spanner cli
...
FLAGS
  ...
  - `--directed-read` = `  DIRECTED_READ  `  
    Enables directed reads to provide the flexibility to route read-only transactions and single reads to a specific replica type or region (replica\_location:replica\_type). The replica\_type is optional and can be either READ\_ONLY or READ\_WRITE.
  - `--execute` = `  EXECUTE  `  
